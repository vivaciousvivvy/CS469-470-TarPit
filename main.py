...
# Fetches or creates session history
def get_session_history(session_id: str) -> BaseChatMessageHistory:
    if session_id not in store:
        store[session_id] = ChatMessageHistory()
    return store[session_id]

# Define persona and instructions
persona = "Fill me in."
...
def butcher_response(request: Request):
    """
    Google Cloud Function to handle Slack slash command requests.
    """
    if request.method != "POST":
    ...
    session_id = data.get("user_id", "default_session")

    if not user_input:
        return jsonify({"text": "Please provide input for the command."})

    # Process the input using the LLM chain
    response = with_message_history.invoke(
